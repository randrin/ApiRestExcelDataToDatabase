...
public class SheetInterface {
	...
	public void createSheetData(Workbook workbook, Sheet sheet, ArrayList<ExcelObjectList> excelObjectList, String title, short indexColorFont, short indexColorBackground, int titleStart, int headerStart){
		createTitleCell(workbook, sheet, title, excelObjectList.size(), indexColorFont, indexColorBackground, titleStart);
		createHeaderCell(workbook, sheet, title, excelObjectList, indexColorFont, indexColorBackground, headerStart);
	}

	public void createTitleCell(Workbook workbook, Sheet sheet,String title, int dimension, short indexColorFont, short indexColorBackground, int titleStart){
	...
	public void createHeaderCell(Workbook workbook, Sheet sheet,String title,ArrayList<ExcelObjectList> excelObjectList, short indexColorFont, short indexColorBackground, int headerStart){
	...
	public ArrayList<ExcelObjectList> getAllGetterMethod(Class c){
		...
		for (Method method : methods){
			if(method.getName().startsWith("get") && method.getAnnotation(ExcelField.class)!=null){
				try{
					ExcelObjectList excelObjectList = new ExcelObjectList(method.getAnnotation(ExcelField.class).position(), method.getAnnotation(ExcelField.class).field(), method);
					list.add(excelObjectList);
				}catch (Exception e) {
					logger.info("Error getting method for class "+ c + "; message: "+ e.getLocalizedMessage() + " " + e.getCause() + " " + e.toString());
				}
			}
		}
	...
	public void populateCell(Sheet sheet, ArrayList<ExcelObjectList> methods, Object objectToInvoke, int rowNum, CellStyle dataCellStyle){
					...
					row.forEach(cell -> {
						try {
							...
							if(methodTypeParams.contains("String")) {
								...
								excelObjectList.get(cell.getColumnIndex()-1).getMethodCaller().invoke(modelEntityObject, valueCell);
							}else {
								excelObjectList.get(cell.getColumnIndex()-1).getMethodCaller().invoke(modelEntityObject, (int)Double.parseDouble(valueCell));
							}

						}catch(Exception e){
							logger.error("Error: " + sheet.getSheetName() + "\t" + row.getRowNum() +"\t" +cell.getColumnIndex() +"\t"+e.getMessage() + "\n" + e);
						}
					});
